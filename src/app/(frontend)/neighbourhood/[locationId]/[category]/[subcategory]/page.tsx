...
export default async function Page({ params }: PageProps) {
  ...
  if (item) {
    // FIX HERE: Remove 'locations.locality' strict filter if it returns 0 items.
    // Get all records in the neighbourhood collection so getRelatedItems function can filter properly.
    const allNeighbourhoodRes = await payload.find({
      collection: 'neighbourhood',
      depth: 2,
      limit: 500,
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: 'Neighbourhood Detail | Super Chennai',
  }
}
