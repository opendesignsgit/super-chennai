...
import SummerFestRegistrationEmail from '../registrtionsComponent'
...
const ArattaiBlockComponent: React.FC<Props> = ({ block, arattaiData }) => {
  ...
  const registrationSettings = arattaiData?.formSettings?.regSettings || {}

  const customFields: CustomField[] = Array.isArray(arattaiData?.formSettings?.customFields)
    ? arattaiData.formSettings.customFields
    : []

  const title = arattaiData?.eventFields?.title || arattaiData?.title || 'Event'
  ...
  const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      payload.append('eventId', String(arattaiData?.id))

      payload.append('slug', String(arattaiData?.slug))
...
export default ArattaiBlockComponent
