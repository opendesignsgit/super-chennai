...
// export default function EventsCalendarBlock({ heading, description, events }: Props) {
...
export default function EventsCalendarBlock({ heading, description, events }: Props) {
  ...
  const slide = (direction: 'left' | 'right') => {
    const distance = 300
    if (direction === 'left') {
      setX((prevX) => Math.min(prevX + distance, 0)) // Prevent sliding too far right
    } else {
      setX((prevX) => Math.max(prevX - distance, -(events.length * 300 - 1200))) // Prevent sliding too far left
    }
  }
  ...
  useEffect(() => {
    console.log('Received events1111111111111111111:', events)
  }, [events])
